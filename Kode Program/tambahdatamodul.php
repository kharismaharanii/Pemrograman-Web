...
session_start();
    ...
    {
        $simpan = mysqli_query($koneksi, "INSERT INTO modul (judul, kelas)
                                            VALUES ('$_POST[judul]',
                                                   ' $_POST[kelas]')
                                                ");
        if($simpan)
        {
            ...
            document.location='tambahdatamodul.php';
            </script>";
        }
        else
        {
  ...
  <body>
    <nav class="navbar navbar-expand-lg bg-dark fixed-top">
      <a class="navbar-brand text-white p-2"><b>ADMINISTRATOR</b></a>
        ...
        <h4>Tambah Data Modul</h4>
        <hr class = bg-secondary>
        <div class="card-body">
            <form method="post">
                <div class="form-grup">
                    <label>Judul Modul</label>
                    <input type="text" name="judul" class="form-control" placeholder="Masukkan Judul" autocomplete="off" required>
                </div>
                ...
                <button type="submit" class="btn btn-success" style ="float : right; margin-right: 4px" name="tombol_simpanmodul">Simpan</a>
            </div>
        </div>
        </div>
      </div>
